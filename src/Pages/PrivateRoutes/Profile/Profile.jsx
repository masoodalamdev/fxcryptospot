import { Avatar, Box, Card, CardHeader, Grid, InputBase, Toolbar, useTheme } from '@mui/material'
...
const checkLoggedUser = async () => {userServices.getLoggedUser(token)
  ...
  return (
    <Box component="main" sx={{ flexGrow: 1, bgcolor: theme.palette.background.default, px:{xs:3, sm:10, md:9, lg:8, xl:32}, minHeight: 100 + 'vh' }} >
      ...
      <PageHeader
        icon={searchBar ? (searchHeader.icon === true ? <SearchIcon size={24} /> : <FcCancel size={24} />) : <AdminPanelSettings size={24} />}
        title={searchBar ? searchHeader.title : "Profile Info"}
        subTitle={searchBar ? searchHeader.subTitle : "Account Management"}
      />
      {searchBar ?
        <InputBase
        autoComplete='off'
          ...
          searchBar ?
            ...
            <Grid item xs={12}>
              <Card>
                <CardHeader
                  avatar={
                    <Avatar aria-label="recipe" src={user && user.image} sx={{ width: 56, height: 56 }}>
